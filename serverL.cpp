...
int main(){
    ...
    while (true){
        ...
        if (buf[4] == '!'){//check if it is an admin request
            admin++;
            buf[4] = '\0';
            cout << "Server L received an inventory status request for code " << buf << ".\n";
        } else {
            cout << "Server L received " << buf << " code from the Main Server.\n";
        }
        int invent = -1;
        ...
        if (invent == -1){//book code not found, tell main server
            ...
            if (admin != 0){//send inventory for admin request
                ...
                cout << "Server L finished sending the inventory status to the Main server using UDP on port " << UDPPORT << ".\n";
            } else {
                ...
                cout << "Server L finished sending the availability status of code " << buf << " to the Main Server using UDP on port " << UDPPORT << ".\n";
            }
        }
    }
    return 0;
...
void CreateAndSend(){
    ...
    //must assign ServerL's static port
    ...
    cout << "Server L is up and running using UDP on port " << UDPPORT <<  "." << endl;
    if ((numbytes = sendto(sockfd, "serverL", 7, 0,
             p->ai_addr, p->ai_addrlen)) == -1) {//send to main server so it can save ServerL's address
        perror("talker: sendto");
    }
