...
int main(){
    ...
    while (true){
        ...
        if (buf[4] == '!'){//check if it is an admin request
            admin++;
            buf[4] = '\0';
            cout << "Server S received an inventory status request for code " << buf << ".\n";
        } else {
            cout << "Server S received " << buf << " code from the Main Server.\n";
        }
        int invent = -1;
        for (int i=0; i<lib.size(); i++){
            if (lib[i].code == (string)buf){//book code found
                invent = lib[i].inv;
                if (admin == 0 && lib[i].inv > 0){//decrement if available and if user is not admin
                    lib[i].inv--;
                }
            }
        }
        if (invent == -1){//book code not found, tell main server
            ...
            if (admin != 0){//send inventory for admin request
                ...
                cout << "Server S finished sending the inventory status to the Main server using UDP on port " << UDPPORT << ".\n";
            } else {
                ...
                cout << "Server S finished sending the availability status of code " << buf << " to the Main Server using UDP on port " << UDPPORT << ".\n";
            }
        }
    }
    return 0;
...
void CreateAndSend(){
    ...
    //must assign ServerS's static port
    ...
    cout << "Server S is up and running using UDP on port " << UDPPORT <<  "." << endl;
    if ((numbytes = sendto(sockfd, "serverS", 7, 0,
             p->ai_addr, p->ai_addrlen)) == -1) {//send to main server so it can save ServerS's address
        perror("talker: sendto");
    }
